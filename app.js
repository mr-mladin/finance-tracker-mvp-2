...
const DASHBOARD_LAYOUT_KEY = "finance-tracker-mvp-2.dashboard-layout";
...
const dashboardBlockDefinitions = [
  { id: "balance", title: "Доходы и Расходы", defaultSize: "medium" },
  { id: "plan", title: "Динамика и планы", defaultSize: "medium" },
  { id: "operations", title: "Операции", defaultSize: "wide" },
];
const dashboardSizeOrder = ["compact", "medium", "wide"];
const dashboardSizeLabels = {
  compact: "S",
  medium: "M",
  wide: "W",
};
const dashboardColumns = ["left", "right", "full"];
...
const elements = {
  ...
  customizeDashboardButton: document.querySelector("#customizeDashboardButton"),
  resetDashboardLayoutButton: document.querySelector("#resetDashboardLayoutButton"),
  dashboardCanvas: document.querySelector("[data-dashboard-canvas]"),
...
let dashboardDragStartLayout = null;
let dashboardPreviewTargetId = null;
let dashboardPreviewPlacementKey = null;
let dashboardDropCommitted = false;
let dashboardPointerActive = false;
let dashboardPointerStarted = false;
let dashboardPointerStart = { x: 0, y: 0 };
let dashboardPointerOffset = { x: 0, y: 0 };
let dashboardDragStartRect = null;
let dashboardDragGhost = null;
let dashboardMasonryFrame = null;
...
function init() {
  ...
  elements.customizeDashboardButton?.addEventListener("click", () => setDashboardEditing(!dashboardEditing));
  elements.resetDashboardLayoutButton?.addEventListener("click", resetDashboardLayout);
  elements.dashboardCanvas?.addEventListener("click", handleDashboardControlClick);
  elements.dashboardCanvas?.addEventListener("dragstart", handleDashboardDragStart);
  elements.dashboardCanvas?.addEventListener("dragover", handleDashboardDragOver);
  elements.dashboardCanvas?.addEventListener("drop", handleDashboardDrop);
  elements.dashboardCanvas?.addEventListener("dragend", handleDashboardDragEnd);
  elements.dashboardCanvas?.addEventListener("pointerdown", handleDashboardPointerDown);
  window.addEventListener("pointermove", handleDashboardPointerMove);
  window.addEventListener("pointerup", handleDashboardPointerUp);
  window.addEventListener("pointercancel", handleDashboardPointerCancel);
  elements.dashboardCanvas?.addEventListener("mousedown", handleDashboardPointerDown);
  window.addEventListener("mousemove", handleDashboardPointerMove);
  window.addEventListener("mouseup", handleDashboardPointerUp);
...
function createDefaultDashboardLayout() {
  const columnCounts = { left: 0, right: 0, full: 0 };

  return dashboardBlockDefinitions.map((block, index) => ({
    id: block.id,
    visible: true,
    size: block.defaultSize,
    order: index + 1,
    ...createDashboardPlacementDefaults(block.defaultSize, index, columnCounts),
  }));
}

function loadDashboardLayout() {
  const saved = localStorage.getItem(DASHBOARD_LAYOUT_KEY);
  if (!saved) return createDefaultDashboardLayout();

  try {
    return normalizeDashboardLayout(JSON.parse(saved));
  } catch {
    return createDefaultDashboardLayout();
  }
}

function normalizeDashboardLayout(source) {
  const sourceItems = Array.isArray(source) ? source.filter((item) => item && item.id) : [];
  const sourceMap = new Map(
    sourceItems.map((item) => [item.id, item])
  );
  const hasSavedColumns = sourceItems.some((item) => dashboardColumns.includes(item.column));

  const normalized = dashboardBlockDefinitions
    .map((block, index) => {
      const saved = sourceMap.get(block.id) || {};
      const size = dashboardSizeOrder.includes(saved.size) ? saved.size : block.defaultSize;
      const order = Number.isFinite(Number(saved.order)) ? Number(saved.order) : index + 1;
      const columnOrder = Number.isFinite(Number(saved.columnOrder)) ? Number(saved.columnOrder) : order;
      return {
        id: block.id,
        visible: saved.visible !== false,
        size,
        order,
        column: dashboardColumns.includes(saved.column) ? saved.column : "",
        columnOrder,
      };
    })
  ...
  if (hasSavedColumns) {
    normalized.forEach((item, index) => {
      if (item.size === "wide") {
        item.column = "full";
      } else if (!["left", "right"].includes(item.column)) {
        item.column = index === 0 ? "left" : "right";
      }
    });
  } else {
    assignLegacyDashboardColumns(normalized);
  }

  return orderDashboardLayoutForGrid(reindexDashboardColumnOrders(normalized));
}

function createDashboardPlacementDefaults(size, index, columnCounts) {
  const column = size === "wide" ? "full" : index === 0 ? "left" : "right";
  columnCounts[column] += 1;
  return {
    column,
    columnOrder: columnCounts[column],
  };
}

function assignLegacyDashboardColumns(items) {
  const columnCounts = { left: 0, right: 0, full: 0 };
  let primaryColumnFilled = false;

  items.forEach((item) => {
    if (item.size === "wide") {
      item.column = "full";
      item.columnOrder = ++columnCounts.full;
      primaryColumnFilled = false;
      return;
    }

    item.column = primaryColumnFilled ? "right" : "left";
    item.columnOrder = ++columnCounts[item.column];
    primaryColumnFilled = true;
  });
}

function reindexDashboardColumnOrders(items) {
  dashboardColumns.forEach((column) => {
    items
      .filter((item) => item.column === column)
      .sort((a, b) => a.columnOrder - b.columnOrder || a.order - b.order)
      .forEach((item, index) => {
        item.columnOrder = index + 1;
      });
  });
...
function orderDashboardLayoutForGrid(items) {
  const result = [];
  let rowGroup = [];
  ...
  ordered.forEach((item) => {
    if (item.size === "wide" || item.column === "full") {
      flushRowGroup();
      result.push({ ...item, column: "full" });
      return;
    }
...
function placeCompactOperationsBesideFirst(layout) {
  const source = normalizeDashboardLayout(layout);
  const operationsIndex = source.findIndex((item) => item.id === "operations");
  if (operationsIndex < 0) return source;

  const operations = source[operationsIndex];
  if (operations.size !== "compact" || operations.visible === false) return source;

  const primaryItems = source.filter((item) => item.id !== "operations");
  const firstPrimary = primaryItems.find((item) => item.visible !== false) || primaryItems[0];
  if (!firstPrimary) return source;

  const pairedColumn = firstPrimary.column === "right" ? "left" : "right";
  source[operationsIndex] = {
    ...operations,
    column: pairedColumn,
    columnOrder: firstPrimary.columnOrder,
  };
  return normalizeDashboardLayout(source);
}

function swapDashboardBlocks(layout, sourceId, targetId) {
  if (!sourceId || !targetId || sourceId === targetId) return normalizeDashboardLayout(layout);
  const nextLayout = normalizeDashboardLayout(layout);
  const sourceIndex = nextLayout.findIndex((item) => item.id === sourceId);
  const targetIndex = nextLayout.findIndex((item) => item.id === targetId);
  if (sourceIndex < 0 || targetIndex < 0) return nextLayout;
  [nextLayout[sourceIndex], nextLayout[targetIndex]] = [nextLayout[targetIndex], nextLayout[sourceIndex]];
  return nextLayout.map((item, index) => ({ ...item, order: index + 1 }));
}

function setDashboardEditing(enabled) {
  dashboardEditing = Boolean(enabled);
...
function resetDashboardLayout() {
  ...
  saveDashboardLayout();
  applyDashboardLayout();
  showToast("Вид главного экрана сброшен");
}

function applyDashboardLayout() {
  if (!elements.dashboardCanvas) return;

  dashboardLayout = normalizeDashboardLayout(dashboardLayout);
  document.body.classList.toggle("dashboard-editing", dashboardEditing);
  elements.customizeDashboardButton?.classList.toggle("active", dashboardEditing);
  elements.customizeDashboardButton?.setAttribute("aria-pressed", String(dashboardEditing));
  elements.resetDashboardLayoutButton?.classList.toggle("hidden", !dashboardEditing);

  const blocks = new Map(
    [...elements.dashboardCanvas.querySelectorAll("[data-dashboard-block]")]
      .map((node) => [node.dataset.dashboardBlock, node])
  );

  dashboardLayout.forEach((item) => {
    const block = blocks.get(item.id);
    if (block) elements.dashboardCanvas.append(block);
  });

  dashboardLayout.forEach((item) => {
    const block = blocks.get(item.id);
    if (!block) return;
    block.style.removeProperty("--dashboard-grid-column");
    block.classList.remove("dashboard-size-compact", "dashboard-size-medium", "dashboard-size-wide");
    block.classList.add(`dashboard-size-${item.size}`);
    block.classList.toggle("dashboard-block-hidden", !item.visible && !dashboardEditing);
    block.classList.toggle("dashboard-block-muted", !item.visible && dashboardEditing);
    block.setAttribute("draggable", "false");
    block.dataset.dashboardVisible = String(item.visible);
    block.dataset.dashboardSize = item.size;
    block.dataset.dashboardColumn = item.column;
  });

  applyDashboardGridPlacement(blocks);
  renderDashboardBlockControls();
  scheduleDashboardMasonry();
}

function applyDashboardGridPlacement(blocks) {
  const flowLayout = dashboardLayout.filter((item) => item.visible !== false || dashboardEditing);

  flowLayout.forEach((item) => {
    const block = blocks.get(item.id);
    if (!block) return;

    block.style.setProperty("--dashboard-grid-column", getDashboardGridColumn(item));
  });
}

function getDashboardGridColumn(item) {
  if (item.size === "wide" || item.column === "full") return "1 / -1";
  const span = item.size === "compact" && item.id !== "operations" ? 5 : 6;
  return item.column === "right" ? `7 / span ${span}` : `1 / span ${span}`;
}

function renderDashboardBlockControls() {
  if (!elements.dashboardCanvas) return;

  const blocks = [...elements.dashboardCanvas.querySelectorAll("[data-dashboard-block]")];
  blocks.forEach((block) => {
    const id = block.dataset.dashboardBlock;
    const item = dashboardLayout.find((layoutItem) => layoutItem.id === id);
    const definition = dashboardBlockDefinitions.find((candidate) => candidate.id === id);
    if (!item || !definition) return;

    let controls = block.querySelector(".dashboard-block-controls");
    if (!controls) {
      controls = document.createElement("div");
      controls.className = "dashboard-block-controls";
      controls.setAttribute("data-dashboard-controls", "");
      block.prepend(controls);
    }

    const index = dashboardLayout.findIndex((layoutItem) => layoutItem.id === id);
    controls.innerHTML = `
      <span>${escapeHtml(definition.title)}</span>
      <div class="dashboard-control-group">
        <span class="dashboard-drag-handle" data-dashboard-drag-handle aria-hidden="true">⋮⋮</span>
        <button type="button" data-dashboard-action="move-up" aria-label="Поднять блок" ${index === 0 ? "disabled" : ""}>↑</button>
        <button type="button" data-dashboard-action="move-down" aria-label="Опустить блок" ${index === dashboardLayout.length - 1 ? "disabled" : ""}>↓</button>
        <button type="button" data-dashboard-action="cycle-size" aria-label="Изменить размер">${dashboardSizeLabels[item.size] || "M"}</button>
        <button type="button" data-dashboard-action="toggle-visibility" aria-label="${item.visible ? "Скрыть блок" : "Показать блок"}">${item.visible ? "◐" : "○"}</button>
      </div>
    `;
  });
}

function handleDashboardControlClick(event) {
  ...
  if (action === "move-up") moveDashboardBlock(id, -1);
  if (action === "move-down") moveDashboardBlock(id, 1);
  if (action === "cycle-size") cycleDashboardBlockSize(id);
  if (action === "toggle-visibility") toggleDashboardBlockVisibility(id);

  saveDashboardLayout();
  applyDashboardLayout();
}

function moveDashboardBlock(id, delta) {
  const index = dashboardLayout.findIndex((item) => item.id === id);
  const nextIndex = index + delta;
  if (index < 0 || nextIndex < 0 || nextIndex >= dashboardLayout.length) return;

  const nextLayout = [...dashboardLayout];
  const [item] = nextLayout.splice(index, 1);
  nextLayout.splice(nextIndex, 0, item);
  dashboardLayout = normalizeDashboardLayout(nextLayout.map((layoutItem, orderIndex) => ({ ...layoutItem, order: orderIndex + 1 })));
}

function moveDashboardBlockBefore(sourceId, targetId) {
  moveDashboardBlockNear(sourceId, targetId, false);
}

function moveDashboardBlockNear(sourceId, targetId, placeAfter = false) {
  ...
  nextLayout.splice(placeAfter ? targetIndex + 1 : targetIndex, 0, source);
  dashboardLayout = normalizeDashboardLayout(nextLayout.map((item, index) => ({ ...item, order: index + 1 })));
}

function cycleDashboardBlockSize(id) {
  let nextSize = null;
  dashboardLayout = dashboardLayout.map((item) => {
    if (item.id !== id) return item;
    const currentIndex = dashboardSizeOrder.indexOf(item.size);
    nextSize = dashboardSizeOrder[(currentIndex + 1) % dashboardSizeOrder.length] || "medium";
    return {
      ...item,
      size: nextSize,
      column: nextSize === "wide" ? "full" : item.column === "full" ? "left" : item.column,
    };
  });

  if (id === "operations" && nextSize === "compact") {
    dashboardLayout = placeCompactOperationsBesideFirst(dashboardLayout);
  } else {
    dashboardLayout = normalizeDashboardLayout(dashboardLayout);
  }
}

function toggleDashboardBlockVisibility(id) {
  const item = dashboardLayout.find((layoutItem) => layoutItem.id === id);
  if (!item) return;
  const visibleCount = dashboardLayout.filter((layoutItem) => layoutItem.visible).length;
  if (item.visible && visibleCount <= 1) {
    showToast("Оставь хотя бы один блок на главном экране");
    return;
  }
...
function handleDashboardDragStart(event) {
  ...
  if (!block || (event.target.closest("[data-dashboard-controls]") && !handle)) return;

  dashboardDragId = block.dataset.dashboardBlock;
  dashboardDragStartLayout = normalizeDashboardLayout(dashboardLayout);
  dashboardDragStartRect = getDashboardRectSnapshot(block);
  dashboardPreviewTargetId = null;
  dashboardPreviewPlacementKey = null;
  dashboardDropCommitted = false;
  updateDashboardDragClasses();
  event.dataTransfer.effectAllowed = "move";
  event.dataTransfer.setData("text/plain", dashboardDragId);
}

function handleDashboardPointerDown(event) {
  if (dashboardPointerActive || !dashboardEditing || event.button !== 0) return;
  const block = event.target.closest("[data-dashboard-block]");
  const handle = event.target.closest("[data-dashboard-drag-handle]");
  if (!block || (event.target.closest("[data-dashboard-controls]") && !handle)) return;
  ...
  dashboardPreviewPlacementKey = null;
  dashboardDropCommitted = false;
  dashboardPointerActive = true;
  dashboardPointerStarted = false;
  dashboardPointerStart = { x: event.clientX, y: event.clientY };
  dashboardPointerOffset = { x: event.clientX - rect.left, y: event.clientY - rect.top };
}

function handleDashboardPointerMove(event) {
  if (!dashboardPointerActive || !dashboardDragId) return;

  const distance = Math.hypot(event.clientX - dashboardPointerStart.x, event.clientY - dashboardPointerStart.y);
  if (!dashboardPointerStarted && distance < 6) return;

  event.preventDefault();
  window.getSelection?.()?.removeAllRanges();
  if (!dashboardPointerStarted) {
    dashboardPointerStarted = true;
    document.body.classList.add("dashboard-drag-active");
    createDashboardDragGhost();
    updateDashboardDragClasses();
  }

  moveDashboardDragGhost(event);

  if (dashboardPreviewPlacementKey && isDashboardPointerInStartRect(event)) {
    dashboardPreviewTargetId = null;
    dashboardPreviewPlacementKey = null;
    dashboardLayout = normalizeDashboardLayout(dashboardDragStartLayout || dashboardLayout);
    applyDashboardLayout();
    updateDashboardDragClasses();
    return;
  }

  const placement = getDashboardDragPlacement(event);
  if (placement) previewDashboardDragPlacement(placement);
}

function handleDashboardPointerUp(event) {
  if (!dashboardPointerActive) return;
  event.preventDefault();

  if (dashboardPointerStarted && isDashboardPointerInStartRect(event)) {
    dashboardPreviewTargetId = null;
    dashboardPreviewPlacementKey = null;
    if (dashboardDragStartLayout) {
      dashboardLayout = dashboardDragStartLayout;
      applyDashboardLayout();
    }
  ...
  if (dashboardPointerStarted && dashboardPreviewPlacementKey) {
    dashboardDropCommitted = true;
    saveDashboardLayout();
    applyDashboardLayout();
  } else if (dashboardPointerStarted && dashboardDragStartLayout) {
    dashboardLayout = dashboardDragStartLayout;
    applyDashboardLayout();
  }
...
function handleDashboardPointerCancel() {
  if (!dashboardPointerActive) return;
  if (dashboardDragStartLayout) {
    dashboardLayout = dashboardDragStartLayout;
    applyDashboardLayout();
...
function handleDashboardDragOver(event) {
  if (!dashboardEditing || !dashboardDragId) return;
...
function handleDashboardDrop(event) {
  if (!dashboardEditing || !dashboardDragId) return;
  if (isDashboardPointerInStartRect(event)) {
    event.preventDefault();
    if (dashboardDragStartLayout) {
      dashboardLayout = dashboardDragStartLayout;
      applyDashboardLayout();
  ...
  const placement = getDashboardDragPlacement(event);
  if (!placement && !dashboardPreviewPlacementKey) return;
  event.preventDefault();
  if (placement) previewDashboardDragPlacement(placement);
  dashboardDropCommitted = true;
  saveDashboardLayout();
...
function handleDashboardDragEnd() {
  const shouldRevert = dashboardDragStartLayout && !dashboardDropCommitted;
  if (shouldRevert) {
    dashboardLayout = dashboardDragStartLayout;
    applyDashboardLayout();
  }
...
function clearDashboardDragState() {
  dashboardDragGhost?.remove();
  dashboardDragGhost = null;
  document.body.classList.remove("dashboard-drag-active");
  dashboardDragId = null;
  dashboardDragStartLayout = null;
  dashboardPreviewTargetId = null;
  dashboardPreviewPlacementKey = null;
  dashboardDropCommitted = false;
  dashboardPointerActive = false;
  dashboardPointerStarted = false;
  dashboardPointerOffset = { x: 0, y: 0 };
  dashboardDragStartRect = null;
  elements.dashboardCanvas?.querySelectorAll(".dashboard-block-dragging, .dashboard-block-preview-target").forEach((node) => {
    node.classList.remove("dashboard-block-dragging", "dashboard-block-preview-target");
  });
}

function getDashboardDragPlacement(event) {
  if (!dashboardDragId || !elements.dashboardCanvas) return null;
  const source = (dashboardDragStartLayout || dashboardLayout).find((item) => item.id === dashboardDragId);
  if (!source) return null;

  if (!isDashboardTwoColumnMode() || source.size === "wide") {
    return getDashboardLinearDragPlacement(event, source);
  }

  const canvasRect = elements.dashboardCanvas.getBoundingClientRect();
  const column = event.clientX < canvasRect.left + canvasRect.width / 2 ? "left" : "right";
  const columnItems = getDashboardColumnItems(dashboardDragStartLayout || dashboardLayout, column)
    .filter((item) => item.id !== dashboardDragId && (item.visible !== false || dashboardEditing));

  if (!columnItems.length) {
    return { column, targetId: null, placeAfter: true };
  }

  for (const item of columnItems) {
    const block = getDashboardBlockElement(item.id);
    if (!block) continue;
    const rect = block.getBoundingClientRect();
    if (event.clientY < rect.top + rect.height / 2) {
      return { column, targetId: item.id, placeAfter: false };
    }
  }

  return { column, targetId: columnItems[columnItems.length - 1].id, placeAfter: true };
}

function getDashboardLinearDragPlacement(event, source) {
  const target = getDashboardDragTarget(event);
  if (!target) return null;

  const targetId = target.dataset.dashboardBlock;
  if (!targetId || targetId === dashboardDragId) return null;

  const targetItem = (dashboardDragStartLayout || dashboardLayout).find((item) => item.id === targetId);
  const rect = target.getBoundingClientRect();
  return {
    column: source.size === "wide" ? "full" : targetItem?.column || source.column || "left",
    targetId,
    placeAfter: event.clientY > rect.top + rect.height / 2,
  };
}

function previewDashboardDragPlacement(placement) {
  const placementKey = getDashboardPlacementKey(placement);
  if (!placementKey || placementKey === dashboardPreviewPlacementKey) return;

  dashboardPreviewPlacementKey = placementKey;
  dashboardPreviewTargetId = placement.targetId || null;
  dashboardLayout = moveDashboardBlockToPlacement(dashboardDragStartLayout || dashboardLayout, dashboardDragId, placement);
  applyDashboardLayout();
  updateDashboardDragClasses();
}

function moveDashboardBlockToPlacement(layout, sourceId, placement) {
  if (!sourceId || !placement) return normalizeDashboardLayout(layout);
  const nextLayout = normalizeDashboardLayout(layout).map((item) => ({ ...item }));
  const sourceIndex = nextLayout.findIndex((item) => item.id === sourceId);
  if (sourceIndex < 0) return nextLayout;

  const [source] = nextLayout.splice(sourceIndex, 1);
  const targetColumn = source.size === "wide" ? "full" : placement.column === "right" ? "right" : "left";
  source.column = targetColumn;

  if (targetColumn === "full") {
    const targetIndex = nextLayout.findIndex((item) => item.id === placement.targetId);
    nextLayout.splice(targetIndex >= 0 ? targetIndex + (placement.placeAfter ? 1 : 0) : nextLayout.length, 0, source);
    return normalizeDashboardLayout(nextLayout.map((item, index) => ({ ...item, order: index + 1 })));
  }

  const columnItems = nextLayout
    .filter((item) => item.column === targetColumn && item.size !== "wide")
    .sort((a, b) => a.columnOrder - b.columnOrder || a.order - b.order);
  let insertIndex = columnItems.length;

  if (placement.targetId) {
    const targetIndex = columnItems.findIndex((item) => item.id === placement.targetId);
    if (targetIndex >= 0) insertIndex = targetIndex + (placement.placeAfter ? 1 : 0);
  }

  columnItems.splice(insertIndex, 0, source);
  columnItems.forEach((item, index) => {
    item.columnOrder = index + 1;
  });

  nextLayout.push(source);
  return normalizeDashboardLayout(nextLayout);
}

function getDashboardColumnItems(layout, column) {
  return normalizeDashboardLayout(layout)
    .filter((item) => item.column === column && item.size !== "wide")
    .sort((a, b) => a.columnOrder - b.columnOrder || a.order - b.order);
}

function getDashboardPlacementKey(placement) {
  if (!placement?.column) return "";
  return `${placement.column}:${placement.targetId || "empty"}:${placement.placeAfter ? "after" : "before"}`;
}

function isDashboardTwoColumnMode() {
  return window.matchMedia("(min-width: 700px)").matches;
}

function getDashboardRectSnapshot(block) {
  const rect = block.getBoundingClientRect();
  return {
    left: rect.left,
    top: rect.top,
...
function isDashboardPointerInStartRect(event) {
  if (!dashboardDragStartRect || typeof event.clientX !== "number" || typeof event.clientY !== "number") return false;
  const buffer = 8;
  return (
    event.clientX >= dashboardDragStartRect.left - buffer &&
    event.clientX <= dashboardDragStartRect.right + buffer &&
    event.clientY >= dashboardDragStartRect.top - buffer &&
    event.clientY <= dashboardDragStartRect.bottom + buffer
  );
}

function createDashboardDragGhost() {
  const block = getDashboardBlockElement(dashboardDragId);
  if (!block) return;

  dashboardDragGhost?.remove();
  const rect = block.getBoundingClientRect();
  dashboardDragGhost = block.cloneNode(true);
  dashboardDragGhost.classList.add("dashboard-drag-ghost");
  dashboardDragGhost.removeAttribute("id");
  dashboardDragGhost.removeAttribute("data-dashboard-block");
  dashboardDragGhost.querySelectorAll("[id]").forEach((node) => node.removeAttribute("id"));
  dashboardDragGhost.querySelectorAll("[data-dashboard-block]").forEach((node) => node.removeAttribute("data-dashboard-block"));
  dashboardDragGhost.style.width = `${rect.width}px`;
  dashboardDragGhost.style.height = `${rect.height}px`;
  dashboardDragGhost.style.left = `${rect.left}px`;
  dashboardDragGhost.style.top = `${rect.top}px`;
  document.body.append(dashboardDragGhost);
}

function moveDashboardDragGhost(event) {
  if (!dashboardDragGhost) return;
  dashboardDragGhost.style.left = `${event.clientX - dashboardPointerOffset.x}px`;
  dashboardDragGhost.style.top = `${event.clientY - dashboardPointerOffset.y}px`;
}

function getDashboardDragTarget(event) {
  const elementsAtPoint = typeof document.elementsFromPoint === "function"
    ? document.elementsFromPoint(event.clientX, event.clientY)
    : [event.target];

  for (const candidate of elementsAtPoint) {
    const block = candidate.closest?.("[data-dashboard-block]");
    if (block && block.dataset.dashboardBlock !== dashboardDragId) return block;
  }

  const fallback = event.target.closest?.("[data-dashboard-block]");
  return fallback?.dataset.dashboardBlock !== dashboardDragId ? fallback : null;
}

function getDashboardBlockElement(id) {
  return [...elements.dashboardCanvas.querySelectorAll("[data-dashboard-block]")]
    .find((block) => block.dataset.dashboardBlock === id);
}

function updateDashboardDragClasses() {
  elements.dashboardCanvas?.querySelectorAll(".dashboard-block-dragging, .dashboard-block-preview-target").forEach((node) => {
    node.classList.remove("dashboard-block-dragging", "dashboard-block-preview-target");
  });

  getDashboardBlockElement(dashboardDragId)?.classList.add("dashboard-block-dragging");
  getDashboardBlockElement(dashboardPreviewTargetId)?.classList.add("dashboard-block-preview-target");
}

function scheduleDashboardMasonry() {
  if (dashboardMasonryFrame) cancelAnimationFrame(dashboardMasonryFrame);
  dashboardMasonryFrame = requestAnimationFrame(updateDashboardMasonry);
}

function updateDashboardMasonry() {
  dashboardMasonryFrame = null;
  if (!elements.dashboardCanvas) return;
  ...
  const canvasStyles = getComputedStyle(elements.dashboardCanvas);
  const rowHeight = parseFloat(canvasStyles.gridAutoRows) || 8;
  const rowGap = parseFloat(canvasStyles.rowGap) || 0;
  const denominator = rowHeight + rowGap;

  blocks
    .filter((block) => !block.classList.contains("dashboard-block-hidden") && block.offsetParent !== null)
    .forEach((block) => {
      const height = block.getBoundingClientRect().height;
      const span = Math.max(1, Math.ceil((height + rowGap) / denominator));
...
function setActiveView(view) {
  ...
  if (activeView !== "operations" && dashboardEditing) setDashboardEditing(false);
  const titles = {
    operations: "Операции",
    reports: "Отчёт",
    plans: "План",
